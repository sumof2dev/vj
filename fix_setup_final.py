...
# Find the first and last loadProfileChannels definitions
first_start = -1
last_end = -1
...
for i, line in enumerate(lines):
    if "function loadProfileChannels()" in line:
        if first_start == -1:
            first_start = i
    if "function saveProfile(" in line:
        last_end = i
        break

if first_start != -1 and last_end != -1:
    # We delete everything from the first definition to just before saveProfile
    new_content = r"""        function loadProfileChannels() {
            ...
            if (currentProfileMappings.length === 0 || currentProfileMappings.length !== channels.length) {
                ...
                if (isOldFormat) {
                    currentProfileMappings = currentProfileMappings.map(m => ([{
                        ...
                        lfo: { shape: 'sine', bin: 0, speed: 0.1, react: 0.5 },
            ...
            // Final Size Check & Padding / Migration
...
"""
    lines[first_start:last_end] = [new_content + "\n"]
    with open(file_path, 'w') as f:
        f.writelines(lines)
    print(f"Successfully cleaned setup.html. Replaced lines {first_start+1} to {last_end}.")
else:
    print(f"Failed to find markers: start={first_start}, end={last_end}")
    sys.exit(1)
