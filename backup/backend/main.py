...
async def process_audio_queue():
    ...
    while True:
        try:
                ...
                try:
                    ...
                    if current_time - last_log > 0.5:
                        ...
                        if dmx_port or dmx_engine:
                            ...
                            # Monitoring: L1_Dim(1), L1_X(7), L1_Y(8), MH_H(175), MH_Dim(182)
                    ...
                    if dmx_port:
                        # Optimization: Only send as many channels as actually used in stage_config
            ...
            global last_serialized_state, last_broadcast_time, broadcast_version, connected_clients
            
            # Optimization: If no clients are connected, skip the expensive JSON serialization
            if not connected_clients:
                await asyncio.sleep(0.1) # Throttle slightly to avoid tight loop
                return

            # Adaptive Throttle: 30fps active, 1fps (1s interval) if inactive/silent
            is_active = audio_state.get('vol', 0.0) > 0.01 or audio_state.get('beat', False)
            broadcast_interval = 0.033 if is_active else 1.0
