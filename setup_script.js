...
const ROLE_CALIBRATION = {
    ...
    zoom: ['min_dmx', 'max_dmx', 'center', 'pos_limit_range'],
...
let openSections = { channels: true, modes: false, defaults: false, calibration: false, shapes: false, ranges: false, dynamics: false, generics: false, strobes: false, dimmers: false };
...
function renderCalibrationSection() {
    ...
    rolesWithCal.forEach(chName => {
        ...
        knownParams.forEach(paramName => {
            const paramVal = params[paramName] !== undefined ? params[paramName] : (paramName.includes('range') ? [0, 255] : 0);

            if (Array.isArray(paramVal)) {
                html += `<div class="kv-row">
                            <span class="kv-name" style="font-family:monospace; color:var(--accent)">${paramName}</span>
                            <input class="kv-val" type="number" value="${paramVal[0]}" style="width:50px" onchange="updateCalibrationParam('${chName}', '${paramName}', parseFloat(this.value), 0)">
                            <span style="color:var(--text-dim)">–</span>
                            <input class="kv-val" type="number" value="${paramVal[1]}" style="width:50px" onchange="updateCalibrationParam('${chName}', '${paramName}', parseFloat(this.value), 1)">
                        </div>`;
            } else {
                html += `<div class="kv-row">
                            <span class="kv-name" style="font-family:monospace; color:var(--accent)">${paramName}</span>
...
function updateCalibrationParam(chName, paramName, val, idx = -1) {
    if (!currentProfile.calibration[chName]) currentProfile.calibration[chName] = {};

    if (idx === -1) {
        currentProfile.calibration[chName][paramName] = val;
    } else {
        if (!Array.isArray(currentProfile.calibration[chName][paramName])) {
            currentProfile.calibration[chName][paramName] = [0, 255];
        }
        currentProfile.calibration[chName][paramName][idx] = val;
    }
    syncUiToJson();
}
